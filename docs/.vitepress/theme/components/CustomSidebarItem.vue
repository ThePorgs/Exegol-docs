...
const badgeClass = computed(() => {
  if (badgeText.value === 'new !') return 'badge-new'
  if (badgeText.value === 'incomplete') return 'badge-wip'
  return ''
})
    ...
    >
      ...
      >
        <component :is="textTag" class="text">
          ...
          <span v-else-if="props.item.badge === 'new'" class="badge badge-new" title="New feature">
            <Sparkles style="width: 16px; height: 16px;" />
          </span>
          <span v-else-if="badgeText" :class="['badge', badgeClass]">{{ badgeText }}</span>
        </component>
      </VPLink>
