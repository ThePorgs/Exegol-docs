...
If the deleted container was using an outdated image, the wrapper will (by default) try to delete that outdated image automatically (unless this default behavior is changed in the :ref:`exegol configuration file<exegol_configuration>`, which is not advised since disk space is not limited and Exegol images can take up to 30GB).
